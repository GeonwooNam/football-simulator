...
def main():
    ...
    st = Player(player_id=9, position=np.array([45.0, 40.0, 0.0]), team_id=2, max_speed=35.0)
    players = [cb, fb, st]

    ball = Ball(position=fb.position.copy(), home_team_id=2, away_team_id=1)
    ball.owner_player_id = fb.player_id
    ...
    while True:
        ...
        if frame < 5:
            st.update_running_info(move_direction="MOVE_RIGHT", move_intensity="SPRINT")

        if st.position[0] >= 65.0 and not compute:
            compute = True
            st.compute_circle_curve(target_pos=fb.position, block_pos=cb.position, shorten_ratio=0.5)

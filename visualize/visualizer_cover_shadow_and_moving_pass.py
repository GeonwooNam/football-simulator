...
def main():
    ...
    ball = Ball(position=(32.0, 35.0, 0), home_team_id=1, away_team_id=2)
    ball.velocity = np.array([0, 0, 0], dtype=np.float32)

    home_dm1 = Player(player_id=1, position=np.array([30.0, 35.0, 0.0]), team_id=1, stat_dict={"pass": 90.0})
    home_dm2 = Player(player_id=2, position=np.array([30.0, 45.0, 0.0]), team_id=1, stat_dict={"pass": 90.0})
    away_dm = Player(player_id=3, position=np.array([65.0, 40.0, 0.0]), team_id=2, max_speed=25.0)
    home_st = Player(player_id=4, position=np.array([80.0, 40.0, 0.0]), team_id=1, max_speed=45.0)

    players = Player.player_list
    Player.add_home_away_and_view_ball(ball)

    ball.set_owner(home_dm1)
    away_dm.start_marking(home_st, offset=[15, 0], cover_shadow=True)
    ...
    while True:
        ...
        elif frame >= 600 or home_st.ball is not None:
            home_st.update_running_info(move_intensity="IDLE")

        # 마지막 공격수에게 패스
        if frame == 500:
            ball_owner = [p for p in home_dm_list if p.player_id == ball.owner_player_id][0]
            ball.pass_to(player_from=ball_owner, player_to=home_st, power=30.0, target_offset=[0, -15])

        # home_st.position[0] += 2.0 * DT  # 0.5m/s 속도
        away_dm.update_marking(DT)

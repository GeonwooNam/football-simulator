...
def main():
    ...
    ball = Ball(position=(32.0, 25.0, 0), home_team_id=1, away_team_id=2)
    ball.velocity = np.array([0, 0, 0], dtype=np.float32)

    home_dm1 = Player(player_id=1, position=np.array([30.0, 25.0, 0.0]), team_id=1, stat_dict={"pass": 90.0})
    home_dm2 = Player(player_id=2, position=np.array([30.0, 40.0, 0.0]), team_id=1, stat_dict={"pass": 90.0})
    away_dm = Player(player_id=3, position=np.array([65.0, 35.0, 0.0]), team_id=2, max_speed=30.0)
    home_st = Player(player_id=4, position=np.array([80.0, 30.0, 0.0]), team_id=1, max_speed=35.0)

    players = Player.player_list
    Player.add_home_away_and_view_ball(ball)

    ball.set_owner(home_dm1)
    away_dm.start_marking(home_st, offset=[15, 0], cover_shadow=True, update_interval=2.0)

    home_dm_list = [home_dm1, home_dm2]
    ...
    while True:
        ...
        if 0 < frame < 600 and frame % 80 == 0:
            ball_owner = [p for p in home_dm_list if p.player_id == ball.owner_player_id][0]
            other_dm = [p for p in home_dm_list if p.player_id != ball.owner_player_id][0]
            ball.pass_to(player_from=ball_owner, player_to=other_dm, power=10.0)

        if 80 <= frame < 550:
            value = frame // 80
            if value % 4 == 0:
                home_st.update_running_info(move_direction="MOVE_TOP", move_intensity="LOW_INTENSITY_RUNNING")
            elif value % 4 == 2:
                home_st.update_running_info(move_direction="MOVE_BOTTOM", move_intensity="LOW_INTENSITY_RUNNING")
            else:
                home_st.update_running_info(move_intensity="IDLE")

        if 560 < frame < 650:
            home_st.update_running_info(move_direction="MOVE_TOP", move_intensity="SPRINT")
        elif frame >= 650 or home_st.ball is not None:
            home_st.update_running_info(move_intensity="IDLE")

        if home_st.ball is not None and not home_st.is_ball_angle_attack_side():
            home_st.rotate_towards(DT, degree=0)

        # 마지막 공격수에게 패스
        if frame == 650:
            ball_owner = [p for p in home_dm_list if p.player_id == ball.owner_player_id][0]
            ball.pass_to(player_from=ball_owner, player_to=home_st, power=40.0, target_offset=[0, 0])

        away_dm.update_marking(DT, move_intensity="HIGH_INTENSITY_RUNNING")
        print(away_dm.move_intensity)

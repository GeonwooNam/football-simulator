...
def main():
    ...
    cb = Player(player_id=1, position=np.array([80.0, 34.0, 0.0]), team_id=2)
    st = Player(player_id=9, position=np.array([65.0, 40.0, 0.0]), team_id=1)

    players = Player.player_list
    Player.add_home_away_and_view_ball(ball)

    cb.start_marking(st, offset=[5, 0])
    ...
    while True:
        ...
        st.position[0] += 2.0 * DT  # 0.5m/s 속도
        cb.update_marking(DT)

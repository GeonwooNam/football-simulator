...
def main():
    ...
    away_dm = Player(player_id=3, position=np.array([75.0, 40.0, 0.0]), team_id=2, max_speed=15.0)
    home_st = Player(player_id=4, position=np.array([80.0, 40.0, 0.0]), team_id=1, max_speed=50.0)

    players = Player.player_list
    Player.add_home_away_and_view_ball(ball)

    ball.set_owner(home_dm1)
    away_dm.start_marking(home_st, offset=[5, 0], cover_shadow=True)
    ...
    while True:
        ...
        if frame == 500:
            ball_owner = [p for p in home_dm_list if p.player_id == ball.owner_player_id][0]
            ball.pass_to(player_from=ball_owner, player_to=home_st, power=20.0, is_lob=True, lob_vector=0.3)

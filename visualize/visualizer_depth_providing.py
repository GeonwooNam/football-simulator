...
def main():
    ...
    ball = Ball(position=(Field.REAL_WIDTH/2, Field.REAL_HEIGHT/2, 0), home_team_id=1, away_team_id=2)
    ball.velocity = np.array([0, 0, 0], dtype=np.float32)

    defender_1 = Player(player_id=1, position=np.array([77.0, 20.0, 0.0]), team_id=2)
    defender_2 = Player(player_id=2, position=np.array([78.0, 30.0, 0.0]), team_id=2)
    defender_3 = Player(player_id=3, position=np.array([81.0, 40.0, 0.0]), team_id=2)
    defender_4 = Player(player_id=4, position=np.array([75.0, 50.0, 0.0]), team_id=2)
    striker = Player(player_id=5, position=np.array([65.0, 35.0, 0.0]), team_id=1, max_speed=34.0)
    ...
    while True:
        ...
        if 10 < frame < 100:
            striker.depth_providing(x_offset=10.0, move_intensity="SPRINT")

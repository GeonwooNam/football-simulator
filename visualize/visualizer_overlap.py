...
def main():
    ...
    ball = Ball(position=(81.0, 11.0, 0), home_team_id=1, away_team_id=2)
    ball.velocity = np.array([0, 0, 0], dtype=np.float32)

    defender = Player(player_id=2, position=np.array([83.0, 17.0, 0.0]), team_id=2, max_speed=35.0)
    winger = Player(player_id=3, position=np.array([80.0, 10.0, 0.0]), team_id=1, max_speed=30.0)
    overlap_fullback = Player(player_id=4, position=np.array([65.0, 20.0, 0.0]), team_id=1, max_speed=33.0)

    players = Player.player_list
    Player.add_home_away_and_view_ball(ball)

    ball.set_owner(winger)
    ...
    while True:
        ...
        if winger.ball is not None and not winger.ball_angle() % 360 != -30 % 360:
            winger.rotate_towards(DT, degree=-30)

        if not overlap and frame > 100:
            overlap = True
            overlap_fullback.compute_overlap_curve(winger, outside_offset=6.0, shorten_ratio=0.5, move_intensity="SPRINT")

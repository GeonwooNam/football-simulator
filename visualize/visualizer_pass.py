...
def main():
    ...
    ball = Ball(position=(32, 34, 0), home_team_id=1, away_team_id=2)
    ball.velocity = np.array([0, 0, 0], dtype=np.float32)

    player_1 = Player(position=(30, 34, 0), team_id=1, player_id=1)
    player_2 = Player(position=(70, 34, 0), team_id=2, player_id=2)
    ...
    while True:
        ...
        if frame == 80 and ball.owner_player_id == player_1.player_id:
            ball.pass_to(player_from=player_1, player_to=player_2, power=30.0, target_offset=(0, 0))

        if frame >= 300 and ball.owner_player_id == player_2.player_id:
            ball.pass_to(player_from=player_2, player_to=player_1, power=10.0, target_offset=(0, 0), is_lob=True, lob_vector=0.4)

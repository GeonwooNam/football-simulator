...
def main():
    ...
    ball = Ball(position=(12, 34, 0), home_team_id=1, away_team_id=2)
    ball.velocity = np.array([0, 0, 0], dtype=np.float32)

    player_1 = Player(position=(10, 34, 0), team_id=1, player_id=1)
    ...
    while True:
        ...
        if frame == 80 and ball.owner_player_id == player_1.player_id:
            ball.pass_to(player_from=player_1, player_to=player_2, power=40.0, target_offset=(0, 0), pass_accuracy_rating=90.0)

        if frame >= 300 and ball.owner_player_id == player_2.player_id:
            ball.pass_to(player_from=player_2, player_to=player_1, power=20.0, target_offset=(0, 0), is_lob=True, lob_vector=0.40)
